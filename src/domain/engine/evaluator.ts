...
export function evaluatePricingDsl(
  basePrice: number,
  dsl: DynamicPricingDsl,
...
function shouldApplyRule(
  ...
  if (!rule.conditions) return true; // No conditions means always apply
...
function applyAction(price: number, action: RuleAction): number {
