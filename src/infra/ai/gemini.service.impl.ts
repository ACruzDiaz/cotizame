...
Tu trabajo es identificar qué datos faltan en la siguiente estructura: ${JSON.stringify(
      actualParams
    )}.
...
- Devuelve únicamente los datos en el formato de la estructura indicada.
  ...
  public async getClientIntention(
    clientMessage: string
...
- Devuelve únicamente los datos en el formato de la estructura indicada.
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        responseMimeType: "application/json",
        responseJsonSchema: Intention,
      },
    });
    if (response.text === undefined)
      throw new Error("El agente IA no pudo determinar la intención");

    if (Object.values(Intention).includes(response.text as Intention)) {
      return response.text as Intention;
    } else {
      return null;
    }
  }
